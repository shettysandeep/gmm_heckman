...
tab indcode, gen(ind)
gen bigfirm=firmsize==3
log using gmm_male_bach, replace
*/

use gmm_male_bach.dta,clear
local xlist "tage,marry,efnp, child18,faminc,bigfirm,constant"
local zlist "tage,sqage,marry,metro,bigfirm,constant"
tomata
mata
...
optimize_init_conv_maxiter(S, 120000)
...
*variance = 1/n*inverse((T'*Psi*T))
  *First calculating Psi in the Final Var-covar matrix /// changed p3step to p (first step)
init1hat2=p
bhat1=p[1,1..xc]
chat12=p[1,(xc+1)..(suc+1)]
chat13=p[1,(suc+2)..(sxz+2)]
mu1hat1=X*bhat1'
...
sebdmale=(sqrt(diagonal(cormat)))'
sebdmale2=(sqrt(diagonal(cormat2)))'
resul=p\p3step\sebdmale\sebdmale2
*** End of Code

lwret=Z[.,1..cols(Z)]*p[1,(xc+1)..(suc)]'
lwnret=Z[.,1..cols(Z)]*p[1,(suc+2)..(sxz+1)]'
lwdiff=lwret-lwnret

lwret1=Z[.,1..cols(Z)]*p3step[1,(xc+1)..(suc)]'
lwnret1=Z[.,1..cols(Z)]*p3step[1,(suc+2)..(sxz+1)]'
...
st_matrix("se_st",seadmale2)
st_matrix("cov_st_second",cormat)
st_matrix("se_st_second",seadmale)
st_addvar("double","lwret")
st_store(.,"lwret",lwret)
st_addvar("double","lwnret")
st_store(.,"lwnret",lwnret)
st_addvar("double","lwdiff")
st_store(.,"lwdiff",lwdiff)
st_addvar("double","lwdiffp3")
st_store(.,"lwdiffp3",lwdiff1)

mata matsave gmm_male_bach * , replace //saves all mata workspace as gmm_ad_male.mmat
*mata matuse gmm_male_bach // retrieves for future work
